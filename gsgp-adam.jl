...
function experiment(dname, n, outfile, num_gen, learning_rate, population_size, mom, k1, k2)
...
function main(args)
    ...
    learning_rate = parse(Float64, args[1])
    p1 = parse(Int64, args[2])
    p2 = parse(Int64, args[3])
    ...
    for dname in ["yacht", "bioav", "slump", "toxicity",  "airfoil", "concrete", "ppb"]
        ...
        for i=1:30
            for (k1, k2) in p
                ...
                path1 = "results2-adam" * "-$(learning_rate)"
                    if isdir(path1) == 0
                        mkdir(path1)
                    end
                path2 = path1 * "/$(dname)"
                ...
                if !isfile(path * "/results-$(i)")
                    ...
                    experiment(dname, i, path * "/results-$(i)", num_gen, learning_rate, population_size, mom, k1, k2)
                end
            end
        end
    end       
